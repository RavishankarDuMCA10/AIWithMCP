...
from .models import (
    LocationInfo, CurrentWeather, WeatherForecast, DailyForecast, HourlyForecast, HourlyWeatherPoint
)
...
from .config import (
    MAX_FORECAST_DAYS, MAX_FORECAST_HOURS, HIGH_WIND_THRESHOLD_KMH, SEVERE_WEATHER_CODES, FREEZING_RAIN_CODES, SNOW_CODES
)
...
def register_tools(mcp: FastMCP):
    ...
    @mcp.tool()
    async def search_location_tool(location_name: str, limit: int = 5) -> List[LocationInfo]:
        """
    ...
    @mcp.tool()
    async def get_current_weather(location_name: str, temperature_unit: str = "celsius") -> CurrentWeather:
        """
        ...
        weather_data = await get_weather_data(
            ...
            temparature_unit=temperature_unit
        )
    ...
    @mcp.tool()
    async def get_weather_forecast(location_name: str, forecast_days: int = 7, temperature_unit: str = "celsius") -> WeatherForecast:
        """
    ...
    @mcp.tool()
    async def get_hourly_forecast(location_name: str, forecast_hours: int = 24, temperature_unit: str = "celsius") -> HourlyForecast:
        """
        Get hourly weather forecast for a location.
        
        Args:
            location_name: Name of the location (city, region, etc.)
            forecast_hours: Number of forecast hours (1-48, default 24)
            temperature_unit: Temperature unit ("celsius" or "fahrenheit")
        """
    ...
    async def get_weather_alerts(location_name: str) -> Dict[str, Any]:
        ...
        current_params = [
            "weather_code", "wind_speed_10m", "temperature_2m", "precipitation"
        ]
        hourly_params = [
            "weather_code", "wind_speed_10m", "temperature_2m", "precipitation", "wind_gusts_10m"
        ]
        ...
        # Current for sever weather conditions
        current_weather_code = current["weather_code"]

        if current_weather_code in SEVERE_WEATHER_CODES:
            alerts.append({
                "type": "severe_weather",
                "severity": "high",
                "title": "Thunderstorms Warning",
                "description": weather_code_to_description(current_weather_code),
                "time": "current"
            })
        elif current_weather_code in FREEZING_RAIN_CODES:
            alerts.append({
                "type": "severe_weather",
                "severity": "high",
        ...
        elif current_weather_code in SNOW_CODES:
            alerts.append({
        ...
        for i in range(min(24, len(hourly["time"]))):
            ...
            if weather_code in SEVERE_WEATHER_CODES and not any(alert["type"] == "severe_weather" for alert in alerts):
                alerts.append({
                    "type": "severe_weather",
                    "severity": "medium",
                    "title": "Incoming Thunderstorms",
                    "description": f"Thunderstorms expected at {hourly['time'][i]}",
                    "time": hourly["time"][i]
                })
            break
