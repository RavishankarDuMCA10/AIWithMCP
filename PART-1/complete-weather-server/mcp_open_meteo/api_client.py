...
async def search_locations(location_name: str, limit: int = 10) -> List[Dict[str, Any]]:
    ...
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code == 200:
            ...
            raise Exception(f"Geocoding API error: {error_data.get('reason', 'Unknown error')}")
        

async def get_weather_data(latitude: float,
                           longitude: float,
                           current: Optional[List[str]] = None,
                           hourly: Optional[List[str]] = None,
                           daily: Optional[List[str]] = None,
                           forecast_days: int = 7,
                           temparature_unit: str = "celsius",
                           wind_speed_unit: str = "kmh",
                           preciptation_unit: str = "mm") -> Dict[str, Any]:
    """Get weather data from Open-Meteo forecast API"""
    params = {
        "latitude": latitude,
        "longitude": longitude,
        "temperature_unit": temparature_unit,
        "windspeed_unit": wind_speed_unit,
        "precipitation_unit": preciptation_unit,
