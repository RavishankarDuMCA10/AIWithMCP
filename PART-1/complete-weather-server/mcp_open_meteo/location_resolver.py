...
from .api_client import search_location
...
async def resolve_location(location_name: str) -> LocationInfo:
    ...
    """
    locations = await search_location(location_name, limit=10)

    if not locations:
        raise ValueError(f"No locations found for '{location_name}'. Please try a different search term.")
    
    # Always use the first result (most relevent according to the API)
    loc = locations[0]
    return LocationInfo(
        id=loc["id"],
        name=loc["name"],
